...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    def forward(self, x, emb, context=None, add_params = None, context2 = None):
        attn_sets = []
        for layer in self:
...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps=None, context=None, y=None, add_params = None, context2 = None, **kwargs):
        """
        ...
        h = h.type(x.dtype)
        if self.predict_codebook_ids:
            return self.id_predictor(h), add_params
        else:
            return self.out(h),add_params
        """
        h = x.type(self.dtype)
        module_ind = 0
        ...
        if context2 is not None:
            if self.predict_codebook_ids:
                return self.id_predictor(h), attn_sets
            else:
                return self.out(h), attn_sets
        else:
            if self.predict_codebook_ids:
