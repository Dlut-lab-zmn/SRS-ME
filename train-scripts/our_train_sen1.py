...
def train_esd(prompt, train_method, start_guidance, negative_guidance, iterations, lr, config_path, ckpt_path, diffusers_config_path, devices, seperator=None, image_size=512, ddim_steps=50):
    ...
    for i in pbar:
        ...
        with torch.no_grad():
            ...
            z_wc = quick_sample_till_t(emb_wc.to(devices[0]), start_guidance, start_code, int(t_enc)) # emb_p seems to work better instead of emb_0
            z_o = quick_sample_till_t(emb_none.to(devices[0]), start_guidance, start_code, int(t_enc)) # emb_p seems to work better instead of emb_0

            # get conditional and unconditional scores from frozen model at time step t and image z
            e_c = model_orig.apply_model(z_c.to(devices[1]), t_enc_ddpm.to(devices[1]), emb_c.to(devices[1]))
            e_wc = model_orig.apply_model(z_wc.to(devices[1]), t_enc_ddpm.to(devices[1]), emb_wc.to(devices[1]))
            e_none = model_orig.apply_model(z_o.to(devices[1]), t_enc_ddpm.to(devices[1]), emb_none.to(devices[1]))
        # breakpoint()
        ...
        p_e_none = model.apply_model(z_o.to(devices[0]), t_enc_ddpm.to(devices[0]), emb_none.to(devices[0]))
        ...
        loss_neg_tar = criteria(p_e_c[c_ind].to(devices[0]), e_wc[c_ind].to(devices[0]))
        loss_neg_untar = - criteria(p_e_c[c_ind].to(devices[0]) - p_e_wc[c_ind].to(devices[0]), e_c[c_ind].to(devices[0]) - e_wc[c_ind].to(devices[0]))
        ...
        loss = (beta2 * loss_neg_untar + loss_neg_wc + loss_neg_none) + beta * (loss_pos_c + loss_pos_wc + loss_pos_none)
