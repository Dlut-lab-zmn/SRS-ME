...
def train_esd(prompt, train_method, start_guidance, negative_guidance, iterations, lr, config_path, ckpt_path, diffusers_config_path, devices, seperator=None, image_size=512, ddim_steps=50):
    ...
    for i in pbar:
        ...
        start_code_init = torch.randn((batch_size, 4, 64, 64)).to(devices[0])

        start_code = torch.cat((start_code_init, start_code_init[c_ind].unsqueeze()), 0)
        emb_p = torch.cat((emb_c, emb_wc[c_ind].unsqueeze()),0)
        ...
        with torch.no_grad():
            ...
            e_0 = model_orig.apply_model(z[-1].unsqueeze().to(devices[1]), t_enc_ddpm.to(devices[1]), emb_wc[c_ind].unsqueeze().to(devices[1]))
            e_p = model_orig.apply_model(z[:-1].to(devices[1]), t_enc_ddpm.to(devices[1]), emb_c.to(devices[1]))
        # breakpoint()
        # get conditional score from ESD model
        e_n = model.apply_model(z[:-1].to(devices[0]), t_enc_ddpm.to(devices[0]), emb_c.to(devices[0]))
        ...
        loss_pos = criteria(e_n[np.array(prompt_order) != 0].to(devices[0]), e_p[np.array(prompt_order) != 0].to(devices[0]))
        loss_neg = criteria(e_n[c_ind].to(devices[0]), e_0.to(devices[0]))
        loss = loss_pos + beta * loss_neg
