...
def train_esd(prompt, train_method, start_guidance, negative_guidance, iterations, lr, config_path, ckpt_path, diffusers_config_path, devices, seperator=None, image_size=512, ddim_steps=50):
    ...
    df_pos = pd.read_csv(prompts_path_pos)
    df_neg = pd.read_csv(prompts_path_neg)

    prompt_pos_set = []
    prompt_neg_set = []
    for _, row in df_pos.iterrows():
        prompt_pos_set.append(str(row))
    for _, row in df_neg.iterrows():
        prompt_neg_set.append(str(row))
    pbar = tqdm(range(len(prompt_neg_set)))
    p_ec_ewc = 0
    p_ec_enone = 0
    for i in pbar:
        
        pos_1 = random.randint(0, len(df_pos))
        pos_2 = random.randint(0, len(df_pos))

        neg_prompt = prompt_neg_set[i]
        neg_prompt_wc = neg_prompt.replace(prompt, 'artist')

        prompt_order = list(np.arange(batch_size))
        random.shuffle(prompt_order)


        batch_prompt = [neg_prompt, prompt_pos_set[pos_1], prompt_pos_set[pos_2]]
        batch_prompt_wc = [neg_prompt_wc, prompt_pos_set[pos_1], prompt_pos_set[pos_2]]

        emb_c = model.get_learned_conditioning(batch_prompt[np.array(prompt_order)])
        emb_wc = model.get_learned_conditioning(batch_prompt_wc[np.array(prompt_order)])
        emb_none = model.get_learned_conditioning(['']*batch_size)

        c_ind = np.where(np.array(prompt_order) == 0)
        ...
        start_code = torch.randn((batch_size, 4, 64, 64))
        with torch.no_grad():
            # generate an image with the concept from ESD model
            z_c = quick_sample_till_t(emb_c.to(devices[0]), start_guidance, start_code, int(t_enc)) # emb_p seems to work better instead of emb_0

            # get conditional and unconditional scores from frozen model at time step t and image z
            e_c = model_orig.apply_model(z_c.to(devices[1]), t_enc_ddpm.to(devices[1]), emb_c.to(devices[1]))
            e_wc = model_orig.apply_model(z_c.to(devices[1]), t_enc_ddpm.to(devices[1]), emb_wc.to(devices[1]))
            e_none = model_orig.apply_model(z_c.to(devices[1]), t_enc_ddpm.to(devices[1]), emb_none.to(devices[1]))
        # breakpoint()
        # get conditional score from ESD model
        p_e_c = model.apply_model(z_c.to(devices[0]), t_enc_ddpm.to(devices[0]), emb_c.to(devices[0]))
        p_e_wc = model.apply_model(z_c.to(devices[0]), t_enc_ddpm.to(devices[0]), emb_wc.to(devices[0]))
        p_e_none = model.apply_model(z_c.to(devices[0]), t_enc_ddpm.to(devices[0]), emb_none.to(devices[0]))
        e_c.requires_grad = False
        e_wc.requires_grad = False
        e_none.requires_grad = False
        # reconstruction loss for ESD objective from frozen model and conditional score of ESD model
        beta = 5
        beta2 = 0.1

        loss_pos_c = criteria(p_e_c[np.array(prompt_order) != 0].to(devices[0]), e_c[np.array(prompt_order) != 0].to(devices[0]))
        loss_pos_wc = criteria(p_e_wc[np.array(prompt_order) != 0].to(devices[0]), e_wc[np.array(prompt_order) != 0].to(devices[0]))
        loss_pos_none = criteria(p_e_none[np.array(prompt_order) != 0].to(devices[0]), e_none[np.array(prompt_order) != 0].to(devices[0]))
        
        current_ec_ewc = torch.mean(torch.square(e_c - e_wc))
        current_ec_enone = torch.mean(torch.square(e_c - e_none))
        p_ec_ewc += current_ec_ewc
        p_ec_enone += current_ec_enone

        if current_ec_ewc > p_ec_ewc/i:
            loss_neg = current_ec_ewc/(p_ec_ewc/i) * criteria(p_e_c[c_ind].to(devices[0]), e_wc[c_ind].to(devices[0]))
        elif current_ec_enone > p_ec_enone/i:
            loss_neg = criteria(p_e_c[c_ind].to(devices[0]), e_none[c_ind].to(devices[0]))
        else:
            loss_neg = - criteria(p_e_c[c_ind].to(devices[0]) - p_e_wc[c_ind].to(devices[0]), e_c[c_ind].to(devices[0]) - e_wc[c_ind].to(devices[0]))

        loss_neg_wc = criteria(p_e_wc[c_ind].to(devices[0]), e_wc[c_ind].to(devices[0]))
        loss_neg_none = criteria(p_e_none[c_ind].to(devices[0]), e_none[c_ind].to(devices[0]))

        loss = (beta2 * loss_neg + loss_neg_wc + loss_neg_none) + beta * (loss_pos_c + loss_pos_wc + loss_pos_none)
