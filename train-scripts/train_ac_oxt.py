...
def train_esd(prompt, train_method, start_guidance, negative_guidance, iterations, lr, config_path, ckpt_path, diffusers_config_path, devices, seperator=None, image_size=512, ddim_steps=50):
    ...
    for i in pbar:
        ...
        start_code = torch.cat((start_code_init, start_code_init), 0)
        emb_p = torch.cat((emb_c, emb_wc),0)
        ...
        with torch.no_grad():
            ...
            e_0 = model_orig.apply_model(z[-1].unsqueeze().to(devices[1]), t_enc_ddpm.to(devices[1]), emb_wc.to(devices[1]))
        ...
        e_n = model.apply_model(z[0].unsqueeze.to(devices[0]), t_enc_ddpm.to(devices[0]), emb_c.to(devices[0]))
