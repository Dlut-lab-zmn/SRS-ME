...
def _compute_statistics_of_path(path, model, batch_size, dims, cuda):
    if path.endswith('.npz'):
        ...
        if (io.imread(str(files[0])).astype(np.float32)).ndim ==2:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-c', '--gpu', default='7', type=str,
    ...
    for i in range(9):
        ...
        fid_value = calculate_fid_given_paths(original_path,
                                            edited_path,
                                            args.batch_size,
                                            args.gpu != '',
                                            args.dims)
        print(f'FID: {i}', fid_value)
